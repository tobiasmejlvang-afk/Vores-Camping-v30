...
const emptyState = () => ({
  version: APP_VERSION,
  settings: {
    appName:'Vores Camping', nextTrip:'', homeLayout:'normal', mapStyle:'liberty', satelliteStyle:'',
    ratingCategories: DEFAULT_RATINGS.map(([id,label,icon])=>({id,label,icon})), autoVacationWatch:true,
    compressImages:true, imageQuality:.82, imageMax:1800, hgv:{length:'',width:'',height:'',weight:'',axleload:''}
  },
  campgrounds:[], visits:[], vacations:[], routes:[], people:[], pets:[], notes:[], experiences:[], photos:[]
});
let state = loadState();
let routeContext = {};
let activeMaps = [];
...
function loadState(){
  ...
  for(const k of ['campgrounds','visits','vacations','routes','people','pets','notes','experiences','photos']) if(!Array.isArray(s[k])) s[k]=[];
...
function bindGlobal(){
  ...
  document.getElementById('hiddenImageInput').addEventListener('change',async e=>{
    ...
    if(link.routeId){const r=state.routes.find(x=>x.id===link.routeId);if(r)r.photoIds=[...new Set([...(r.photoIds||[]),...ids])];}
...
function handleAction(act,el){
  const map={
    ...
    'new-cycle':()=>openRouteForm('cycling'),'add-photo':()=>openPhotoPicker(), 'add-note':openNoteForm,'start-vacation':openVacationForm,'open-map':()=>setRoute('kort')
  }; if(map[act]) map[act]();
}
function openQuickAdd(){
  modal('Hurtig tilføj',`<div class="quick-grid" style="grid-template-columns:repeat(2,1fr)">
    ${[['add-camp','map-pin-plus','Campingplads'],['add-wish','bookmark-plus','Ønskested'],['add-visit','calendar-plus','Besøg'],['new-cycle','bike','Cykelrute'],['add-photo','image-plus','Billede'],['add-note','notebook-pen','Notat'],['start-vacation','palmtree','Ferie'],['open-map','map','Stort kort']].map(([a,i,l])=>`<button class="quick-card" data-action="${a}">${icon(i)}<b>${l}</b></button>`).join('')}</div>`,{size:'sm'});
}
...
function bindDashboard(){
  const tick=()=>{const n=new Date(); const c=document.getElementById('dashClock'), d=document.getElementById('dashDate'); if(c)c.textContent=n.toLocaleTimeString('da-DK',{hour:'2-digit',minute:'2-digit'}); if(d)d.textContent=n.toLocaleDateString('da-DK',{weekday:'long',day:'numeric',month:'long'});}; tick(); setTimeout(tick,30000);
...
function campFormHTML(c,status){
  const tags=(c?.tags||[]).join(', '); return `<form id="campForm"><div class="form-grid">
    ...
    <div class="field"><label>Hjemmeside</label><input name="website" type="url" value="${esc(c?.website||'')}"></div><div class="field"><label>Telefon</label><input name="phone" value="${esc(c?.phone||'')}"></div>
...
function renderCampDetail(id){
  ...
  document.getElementById('mainContent').innerHTML=`<div class="page">
    ...
    <div class="toolbar"><button class="btn btn-primary" id="editCamp">${icon('pencil')} Redigér</button><button class="btn btn-secondary" id="campAddVisit">${icon('calendar-plus')} Tilføj besøg</button><button class="btn btn-secondary" id="campAddPhoto">${icon('image-plus')} Billede</button>${c.lat&&c.lon?`<button class="btn btn-secondary" id="nearbyCamp">${icon('scan-search')} Området omkring</button><a class="btn btn-ghost" target="_blank" rel="noopener" href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${c.lat},${c.lon}`)}">${icon('external-link')} Google Maps</a>`:''}<button class="btn btn-danger" id="deleteCamp">${icon('trash-2')} Slet</button></div>
...
function getMapStyle(){ return state.settings.mapStyle==='satellite'&&state.settings.satelliteStyle ? state.settings.satelliteStyle : (MAP_STYLES[state.settings.mapStyle]||MAP_STYLES.liberty); }
...
function openMapStylePicker(m){
  modal('Vælg kortstil',`<div class="list">${Object.keys(MAP_STYLES).map(k=>`<button class="item" style="width:100%;text-align:left" data-style="${k}"><div class="item-main"><div class="item-title">${k[0].toUpperCase()+k.slice(1)}</div><div class="item-sub">OpenFreeMap</div></div>${state.settings.mapStyle===k?icon('check'):''}</button>`).join('')}<button class="item" style="width:100%;text-align:left" data-style="satellite"><div class="item-main"><div class="item-title">Satellit / hybrid</div><div class="item-sub">Bruger den eksterne stil-URL fra Indstillinger</div></div></button></div>`,{size:'sm'});
  document.querySelectorAll('[data-style]').forEach(b=>b.onclick=()=>{const k=b.dataset.style;if(k==='satellite'&&!state.settings.satelliteStyle){closeModal();toast('Tilføj først en satellit MapLibre style-URL under Indstillinger → Kort.','error');setRoute('indstillinger');return;}state.settings.mapStyle=k;saveState();closeModal();try{m?.setStyle(getMapStyle());m?.once('style.load',()=>{addCampMarkers(m,state.campgrounds);addRouteLayers(m,state.routes);});}catch(_){}toast('Kortstilen er skiftet.','success');});
}

function openRouteForm(type='route',id=null){
  const r=id?state.routes.find(x=>x.id===id):null; const a=activeVacation(); const profileDefault=type==='cycling'?'cycling-regular':'driving-car';
  modal(type==='cycling'?'Ny cykelrute':'Planlæg rute',`<form id="routeForm"><div class="route-builder"><div><div class="form-grid">
    ...
    ${type==='cycling'?`<div class="field"><label>Sværhedsgrad</label><select name="difficulty"><option>Let</option><option>Mellem</option><option>Svær</option></select></div><div class="field"><label>Cykeltype</label><select name="bikeType"><option value="regular">Almindelig cykel</option><option value="electric">Elcykel</option></select></div><div class="field"><label>Elcykel rækkevidde start (km)</label><input name="rangeStart" type="number" min="0"></div><div class="field"><label>Rækkevidde slut (km)</label><input name="rangeEnd" type="number" min="0"></div>`:''}
  ...
  let points=(r?.coordinates||[]).map(p=>[Number(p[0]),Number(p[1])]); const rm=initRouteBuilderMap(points,p=>{points.push([p.lng,p.lat]);renderWaypoints();drawBuilderPoints(rm,points);});
  ...
  document.getElementById('routeForm').onsubmit=e=>{e.preventDefault();saveRouteForm(new FormData(e.currentTarget),points,type,id,rm?._lastRouteGeoJSON||r?.geojson||null);}; refreshIcons();
}
...
function saveRouteForm(fd,points,type,id,geo){ let r=id?state.routes.find(x=>x.id===id):null;if(!r){r={id:uid('route'),type,photoIds:[],stops:[],createdAt:new Date().toISOString()};state.routes.push(r);} const summary=geo?.features?.[0]?.properties?.summary||{};Object.assign(r,{name:String(fd.get('name')),date:String(fd.get('date')||today()),profile:String(fd.get('profile')),campgroundId:String(fd.get('campgroundId')||''),vacationId:String(fd.get('vacationId')||''),difficulty:String(fd.get('difficulty')||''),description:String(fd.get('description')||''),avoidFeatures:[fd.get('avoidMotorways')==='on'?'highways':'',fd.get('avoidTolls')==='on'?'tollways':'',fd.get('avoidFerries')==='on'?'ferries':''].filter(Boolean),coordinates:points,geojson:geo,distance:Number(summary.distance)||r.distance||0,duration:Number(summary.duration)||r.duration||0,bikes:type==='cycling'?[{id:uid('bike'),type:String(fd.get('bikeType')||'regular'),rangeStart:Number(fd.get('rangeStart'))||null,rangeEnd:Number(fd.get('rangeEnd'))||null}]:[],updatedAt:new Date().toISOString()});saveState();closeModal();toast('Ruten er gemt og kan bruges på tværs af appen.','success');setRoute('cykelruter');}
...
function vacationCardHTML(v){ const visits=state.visits.filter(x=>x.vacationId===v.id),routes=state.routes.filter(x=>x.vacationId===v.id),notes=state.notes.filter(x=>x.vacationId===v.id);return `<article class="camp-card" data-vac="${v.id}" tabindex="0"><div class="camp-cover" data-vac-photo="${v.photoIds?.[0]||''}"><div class="camp-status"><span class="chip ${v.status==='active'?'active':'done'}">${icon(v.status==='active'?'shield-check':'calendar')} ${v.status==='active'?'Aktiv ferie':'Ferie'}</span></div></div><div class="camp-body"><h3>${esc(v.name)}</h3><div class="item-sub">${fmtDate(v.startDate)}${v.endDate?` – ${fmtDate(v.endDate)}`:''} · ${esc(v.destination||'Destination ikke angivet')}</div><div class="chips"><span class="chip">${visits.length} besøg</span><span class="chip">${routes.length} ruter</span><span class="chip">${notes.length} noter</span></div></div></article>`; }
...
function renderVacationWatch(){
  ...
  document.getElementById('watchOpen')?.addEventListener('click',()=>openVacationDetail(a.id));document.getElementById('watchEnd')?.addEventListener('click',()=>{a.status='done';a.endDate=a.endDate||today();saveState();toast('Ferie Vagten har stemplet ud.','success');render();});refreshIcons();
}

function renderAlbums(){
  const vs=[...state.vacations].filter(v=>v.status!=='active').sort((a,b)=>String(b.startDate).localeCompare(String(a.startDate)));
...
function bindSettings(){
  ...
  document.getElementById('settingMapStyle').onchange=e=>{state.settings.mapStyle=e.target.value;saveState();};document.getElementById('satelliteStyle').onchange=e=>{state.settings.satelliteStyle=e.target.value.trim();saveState();};document.getElementById('orsKey').onchange=e=>{setApiKey(e.target.value);toast('API-nøglen er gemt separat.','success');};
